...
class AirCargoProblem(Problem):
    ...
    def get_actions(self):
        ...
        def unload_actions():
            ...
            """
            unloads = []
            # Iterating over all cargoes, planes and airports to fit Unload(c, p, a) action schema with all
            # required effects and preconditions
            for cargo in self.cargos:
                for plane in self.planes:
                    for airport in self.airports:
    ...
    def result(self, state: str, action: Action):
        ...
        # Object of type FluentState with old state after mapping
        old_state = decode_state(state, self.state_map)

        # Pass all old state's positive and negative fluents to new state, except for those already present in
        # opposite effects collection of given action
        ...
        # Pass all positive and negative action's effects to new state, except for those already saved there
...
def air_cargo_p2() -> AirCargoProblem:
    # Initial variables, pos list and goal list are taken as-is from project manual.
    # Neg list is deduced from studying pos list e. g. if plane P1 is at airport SFO
    # then it can't be in JFK or ATL at the same time.
    # Since there's no mention about 'In(.., ..)' fluent then I'm assuming that no cargo is on any plane yet.
    cargoes = ['C1', 'C2', 'C3']
    planes = ['P1', 'P2', 'P3']
    airports = ['JFK', 'SFO', 'ATL']

    pos = [expr('At(P1, SFO)'), expr('At(P2, JFK)'), expr('At(P3, ATL)'),
           expr('At(C1, SFO)'), expr('At(C2, JFK)'), expr('At(C3, ATL)')]

    neg = [expr('At(P1, JFK)'), expr('At(P1, ATL)'),
    ...
    fluent_state = FluentState(pos, neg)

    goal = [expr('At(C1, JFK)'), expr('At(C2, SFO)'), expr('At(C3, SFO)')]

    return AirCargoProblem(cargoes, planes, airports, fluent_state, goal)


def air_cargo_p3() -> AirCargoProblem:
    # Initial variables, pos list and goal list are taken as-is from project manual.
    # Neg list is deduced from studying pos list e. g. if plane P1 is at airport SFO
    # then it can't be in JFK, ATL or ORD at the same time.
    # Since there's no mention about 'In(.., ..)' fluent then I'm assuming that no cargo is on any plane yet.
    cargoes = ['C1', 'C2', 'C3', 'C4']
    planes = ['P1', 'P2']
    airports = ['JFK', 'SFO', 'ATL', 'ORD']

    pos = [expr('At(P1, SFO)'), expr('At(P2, JFK)'),
